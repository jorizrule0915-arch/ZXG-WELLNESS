...
export const Route = createFileRoute("/products/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          { title: `${loaderData.product.name} — ZXG Wellness` },
          { name: "description", content: loaderData.product.description },
          { property: "og:title", content: `${loaderData.product.name} — ZXG Wellness` },
  ...
  notFoundComponent: () => (
    <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-6">
      <div className="font-display text-5xl text-gradient-gold">Not Found</div>
      <p className="mt-3 text-muted-foreground">This product no longer exists in the atelier.</p>
      <Link
        to="/products"
        className="mt-6 text-[11px] uppercase tracking-luxury text-gold border-b border-gold/40"
      >
        Back to collection
      </Link>
    </div>
  ...
  errorComponent: ({ error, reset }) => (
    <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-6">
      <div className="font-display text-3xl">Something went amiss</div>
              ...
              <div className="flex flex-wrap gap-3">
                {product.variants.map((v) => (
                  <button
                    ...
                    className={`px-4 py-3 text-[11px] uppercase tracking-luxury border transition-colors ${
          ...
          <div className="mt-8">
            ...
            <div className="flex flex-wrap gap-2">
              {product.ingredients.map((b: string) => (
                <span key={b} className="text-[10px] uppercase tracking-luxury border border-gold/30 px-3 py-1 text-foreground/70">
                  {b}
                </span>
              ))}
            </div>
