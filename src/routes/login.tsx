...
function LoginPage() {
  ...
  return (
    <>
      <Helmet><title>Sign In — ZXG Wellness</title></Helmet>
      <div className="min-h-[80vh] flex items-center justify-center px-6 py-16">
        ...
        >
          <div className="text-center mb-10">
            <div className="text-[10px] uppercase tracking-luxury text-gold mb-3">Atelier Access</div>
            <h1 className="font-display text-5xl">{mode === "signin" ? "Welcome back" : "Begin"}</h1>
            <p className="mt-3 text-sm text-muted-foreground">
              {mode === "signin" ? "Enter your credentials to continue." : "Create your private atelier account."}
            </p>
          </div>
          ...
          <div className="bg-charcoal border border-gold/20 p-8 backdrop-blur-sm">
            <form onSubmit={onSubmit} className="space-y-5">
              ...
              <button type="submit" disabled={loading} className="w-full py-4 bg-gold text-obsidian text-[11px] uppercase tracking-luxury font-medium hover:bg-gold-light transition-colors disabled:opacity-50 glow-gold-sm">
...
function Field({
  label, value, onChange, type = "text", canToggleVisibility = false, required,
}: {
  ...
  return (
    <label className="block">
      ...
      <span className="relative block">
        ...
        />
        {canToggleVisibility && (
          <button type="button" onClick={() => setVisible((c) => !c)} aria-label={visible ? "Hide password" : "Show password"} className="absolute right-0 top-1/2 -translate-y-1/2 p-2 text-muted-foreground hover:text-gold transition-colors">
