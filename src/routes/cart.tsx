...
function CartPage() {
  ...
  return (
    <>
      <Helmet><title>Atelier Cart — ZXG Wellness</title></Helmet>
      <div className="mx-auto max-w-5xl px-6 lg:px-10 py-20 md:py-28">
        ...
        {items.length === 0 ? (
          <div className="mt-20 text-center py-20 border-y border-gold/15">
            ...
            <p className="mt-3 text-muted-foreground">Your atelier cart awaits its first treasure.</p>
            <Link to="/products" className="mt-8 inline-block px-8 py-3 border border-gold text-gold text-[11px] uppercase tracking-luxury hover:bg-gold hover:text-obsidian transition-colors">
          ...
          <div className="mt-12 grid lg:grid-cols-[1fr,360px] gap-12">
            <ul className="border-t border-gold/15">
              {items.map((i) => (
                <li key={i.slug} className="border-b border-gold/15 py-6 flex gap-6">
                  <img src={i.image} alt={i.name} className="h-32 w-28 object-cover bg-surface-2" loading="lazy" />
                  <div className="flex-1 flex flex-col">
                    <div className="flex items-start justify-between">
                      ...
                      <button onClick={() => remove(i.slug)} aria-label="Remove" className="text-muted-foreground hover:text-gold">
                        <X className="h-4 w-4" />
                      </button>
                    </div>
                    <div className="mt-auto flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <button onClick={() => setQty(i.slug, i.quantity - 1)} className="h-8 w-8 border border-gold/40 text-gold hover:bg-gold hover:text-obsidian transition flex items-center justify-center">
                      ...
                      <div className="font-display text-xl text-gold">${(i.price * i.quantity).toFixed(0)}</div>
                    </div>
                  </div>
                </li>
              ))}
            ...
            <aside className="bg-charcoal border border-gold/20 p-8 h-fit sticky top-24">
              ...
              <Link to="/checkout" className="mt-8 block w-full text-center py-4 bg-gold text-obsidian text-[11px] uppercase tracking-luxury font-medium hover:bg-gold-light transition-colors glow-gold-sm">
                Proceed to Checkout →
              </Link>
            </aside>
          </div>
