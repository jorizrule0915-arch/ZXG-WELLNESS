    ...
    <div className="-mt-16 min-h-screen bg-obsidian flex">
      <aside className="w-64 shrink-0 border-r border-gold/15 bg-charcoal min-h-screen sticky top-0 hidden md:flex flex-col">
        <div className="px-6 py-8 border-b border-gold/15">
          ...
          <div className="text-[10px] uppercase tracking-luxury text-muted-foreground mt-1">Atelier Admin</div>
        </div>
        <nav className="flex-1 py-6 px-4 space-y-1">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.to} to={item.to} className={`flex items-center gap-3 px-4 py-3 text-[11px] uppercase tracking-luxury transition-all ${active ? "bg-gold text-obsidian" : "text-foreground/70 hover:text-gold hover:bg-surface"}`}>
        ...
        <div className="p-4 border-t border-gold/15">
          <Link to="/" className="flex items-center gap-2 px-4 py-2 text-[10px] uppercase tracking-luxury text-muted-foreground hover:text-gold transition-colors">
      ...
      <div className="md:hidden fixed top-0 inset-x-0 h-14 z-40 bg-charcoal border-b border-gold/15 flex items-center justify-between px-4">
        ...
        <Link to="/" className="text-[10px] uppercase tracking-luxury text-muted-foreground">Exit</Link>
      </div>

      <main className={`flex-1 min-w-0 pt-14 md:pt-0 ${mounted ? "opacity-100" : "opacity-0"} transition-opacity duration-500`}>
        <div className="md:hidden flex border-b border-gold/15 bg-charcoal">
          {navItems.map((item) => {
            const active = item.exact ? pathname === item.to : pathname.startsWith(item.to);
            return (
              <Link key={item.to} to={item.to} className={`flex-1 text-center py-3 text-[10px] uppercase tracking-luxury ${active ? "text-gold border-b-2 border-gold" : "text-muted-foreground"}`}>
                {item.label}
              </Link>
            );
          })}
